...
export function TopicDetail({number,title,lead,sections,next}:{number:string;title:string;lead:string;sections:Section[];next:{href:string;label:string}}){return <Site><main>
  <header className="topic-hero"><div className="container"><Link className="back-link" href={siteHref('/')}>← ポスター展示へ戻る</Link><p className="eyebrow" style={{color:'#0017c1'}}>POSTER / SECTION {number}</p><h1>{title}</h1><p className="lead">{lead}</p></div></header>
  ...
  <section className="next-page"><Link href={siteHref(next.href)}><span>次のポスター項目</span><strong>{next.label}</strong><b>→</b></Link></section>
 </main></Site>}
